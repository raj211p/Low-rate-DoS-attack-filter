 ...
 */
void
got_packet_SlowRead(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
    ...
    //printf("   Src port: %d\n", ntohs(tcp->th_sport));
    //printf("   Dst port: %d\n", ntohs(tcp->th_dport));
    int ws=ntohs(tcp->th_win);
    //printf("   Window size: %d\n",ws);

    if(ws<20){
        cout<<"Possible Slow Read attack detected; window size: "<<ws<<" bytes.\n";
        cout<<"Source IP address: "<<src_ip<<"\n";
        slow_read=1;
        slow_read_ip.insert(src_ip); 
    }
...
void slow_Read()
{
    ...
    pcap_loop(handle, -1, got_packet_SlowRead, NULL);
